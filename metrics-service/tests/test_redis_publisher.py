...
class TestConnection:
    ...
    async def test_connect_failure(self, redis_publisher_instance):
        ...
        with patch('redis.asyncio.Redis.from_url', side_effect=redis.ConnectionError("Connection refused")):
            result = await redis_publisher_instance.connect()
...
class TestPublishing:
    ...
    async def test_publish_not_connected(self, redis_publisher_instance):
        """Should return False when not connected"""
        with patch.object(redis_publisher_instance, '_ensure_connected', AsyncMock(return_value=False)):
            result = await redis_publisher_instance.publish(
                "test:channel",
                MetricsEventType.FULL_SNAPSHOT,
                {"data": "test"}
...
class TestUtilityMethods:
    ...
    async def test_store_last_snapshot_not_connected(self, redis_publisher_instance, sample_snapshot):
        """Should return False when not connected"""
        with patch.object(redis_publisher_instance, '_ensure_connected', AsyncMock(return_value=False)):
            result = await redis_publisher_instance.store_last_snapshot(sample_snapshot)
