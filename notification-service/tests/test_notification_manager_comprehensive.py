...
class TestPersistenceLoad:
    ...
    def test_load_preferences_handles_validation_error(self, notification_manager_instance, tmp_path):
    ...
    def test_load_history_converts_integer_network_id(self, notification_manager_instance, tmp_path):
    ...
    def test_load_scheduled_broadcasts_skips_without_network_id(self, notification_manager_instance, tmp_path):
    ...
    def test_load_global_preferences(self, notification_manager_instance, tmp_path):
        ...
        global_prefs_file.write_text(json.dumps({
            "user-1": {
                "user_id": "user-1",
...
class TestPersistenceSave:
    ...
    def test_save_history_error_handling(self, notification_manager_instance, tmp_path):
        ...
        notification_manager_instance._history.append(
            NotificationRecord(
                notification_id="1", event_id="1", network_id="test",
                channel=NotificationChannel.EMAIL, success=True,
                title="Test", message="Test", priority=NotificationPriority.HIGH
            )
        )
    ...
    def test_save_scheduled_broadcasts_error_handling(self, notification_manager_instance, tmp_path):
...
class TestGlobalPreferencesMigration:
    ...
    def test_migrate_skips_existing_users(self, notification_manager_instance):
        ...
        original_email = notification_manager_instance._global_preferences["existing-user"].email_address
        
        with patch.object(notification_manager_instance, '_save_global_preferences'):
            notification_manager_instance._migrate_users_to_global_preferences()
        
        # Should not be overwritten
        assert notification_manager_instance._global_preferences["existing-user"].email_address == original_email
...
class TestSchedulerTask:
    ...
    async def test_send_scheduled_broadcast_no_users(self, notification_manager_instance):
        ...
        with patch.object(notification_manager_instance, '_get_network_member_user_ids', new_callable=AsyncMock, return_value=[]):
            with patch.object(notification_manager_instance, '_save_scheduled_broadcasts'):
                await notification_manager_instance._send_scheduled_broadcast("test-broadcast")
        
        assert notification_manager_instance._scheduled_broadcasts["test-broadcast"].status == ScheduledBroadcastStatus.FAILED
...
class TestNotificationSending:
    ...
    @pytest.mark.asyncio
    async def test_send_notification_to_network_email_not_configured(self, notification_manager_instance):
    ...
    async def test_send_notification_to_network_with_discord(self, notification_manager_instance):
        ...
        with patch('app.services.notification_manager.send_discord_notification', new_callable=AsyncMock, return_value=mock_record):
            with patch.object(notification_manager_instance, '_save_history'):
                records = await notification_manager_instance.send_notification_to_network(
                    "test-network", event, force=True
                )
    ...
    async def test_send_notification_to_network_members(self, notification_manager_instance):
        ...
        with patch('app.services.notification_manager.is_email_configured', return_value=True):
            with patch('app.services.notification_manager.send_notification_email', new_callable=AsyncMock, return_value=mock_record):
                with patch.object(notification_manager_instance, '_save_history'):
    ...
    @pytest.mark.asyncio
    async def test_send_notification_to_network_members_email_exception(self, notification_manager_instance):
        ...
        with patch('app.services.notification_manager.is_email_configured', return_value=True):
            with patch('app.services.notification_manager.send_notification_email', new_callable=AsyncMock, side_effect=Exception("Send failed")):
                with patch.object(notification_manager_instance, '_save_history'):
                    results = await notification_manager_instance.send_notification_to_network_members(
    ...
    @pytest.mark.asyncio
    async def test_send_notification_to_network_members_discord_exception(self, notification_manager_instance):
        ...
        with patch('app.services.notification_manager.send_discord_notification', new_callable=AsyncMock, side_effect=Exception("Discord failed")):
            with patch.object(notification_manager_instance, '_save_history'):
                results = await notification_manager_instance.send_notification_to_network_members(
                    "test-network", ["user1"], event, force=True
                )
...
class TestTestNotifications:
    ...
    async def test_send_test_email_success(self, notification_manager_instance):
        ...
        with patch('app.services.notification_manager.is_email_configured', return_value=True):
            with patch('app.services.notification_manager.send_test_email', new_callable=AsyncMock, return_value={"success": True}):
    ...
    async def test_send_test_discord_success(self, notification_manager_instance):
        ...
        with patch('app.services.notification_manager.is_discord_configured', return_value=True):
            with patch('app.services.notification_manager.send_test_discord', new_callable=AsyncMock, return_value={"success": True}):
                result = await notification_manager_instance.send_test_notification("test-network", request)
...
class TestHistoryAndStatsAdvanced:
    ...
    def test_get_history_all_networks(self, notification_manager_instance):
        ...
        notification_manager_instance._history.append(
            NotificationRecord(
                notification_id="1", event_id="1", network_id="network-1",
                channel=NotificationChannel.EMAIL, success=True,
                title="Test 1", message="Test", priority=NotificationPriority.HIGH
            )
        )
        notification_manager_instance._history.append(
            NotificationRecord(
                notification_id="2", event_id="2", network_id="network-2",
                channel=NotificationChannel.DISCORD, success=True,
                title="Test 2", message="Test", priority=NotificationPriority.HIGH
            )
        )
    ...
    def test_get_stats_with_failures(self, notification_manager_instance):
        ...
        notification_manager_instance._history.append(
            NotificationRecord(
                notification_id="2", event_id="2", network_id="test",
                channel=NotificationChannel.EMAIL, success=False,
                title="Test", message="Test", priority=NotificationPriority.HIGH,
                timestamp=now,
